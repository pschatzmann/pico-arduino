...
class USBMidi {
    public: 
        ...
        void setup_ubs_descriptors(){
            USBDevice device = USBDevice::instance();
            if (!device.isDone()) {
                device.idVendor(0xCafe).idProduct(0x0001).bcdDevice(0x0100).
                    manufacturer("TinyUSB").product("USBMidi").serials("V0.1").
                    done(true);
            }

            // get existing configuratioin or create one if it does not exist
            USBConfiguration config = device.singleConfiguration();
            if (!config.isDone()) {
                config.bMaxPower(50).
                    bmAttributes(TUSB_DESC_CONFIG_ATT_REMOTE_WAKEUP).
                    name("Default USB Midi Configuration").
                    done(true);
            }


            // we create a new interface
            USBInterface intf = config.createInterface();   
            const uint8_t AUDIO = 0x01;
            const uint8_t MIDISTREAMING = 0x03;
            const uint8_t NOTUSED = 0x00;
            intf.bInterfaceClass(AUDIO).bInterfaceSubClass(MIDISTREAMING).bInterfaceProtocol(NOTUSED);

            // define class specific attributes

            const uint8_t classSpecific[] = {0x07, 0x24, 0x01,0x01, 0x00, 0x00, 0x41 };
            intf.setClassSpecificDescriptor(,)

            // Isochronous endpoint
            //USBEndpoint ep = intf.createEndpoint();
            //ep.wMaxPacketSize(512).bInterval(1).transferType(Isochronous).usageType(DataEndpoint); 
